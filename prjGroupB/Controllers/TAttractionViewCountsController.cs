...
namespace prjGroupB.Controllers
{
    ...
    public class TAttractionViewCountsController : ControllerBase
    {
        ...
        public async Task<ActionResult<IEnumerable<TAttractionViewDTO>>> GetAllViewCount()
        {
            try
            {
        ...
        public async Task<ActionResult<int>> IncreaseViewCount([FromBody] TAttractionIdDTO IdObj)
        {
            try
            {
                ...
                DateTime timeLimit = DateTime.UtcNow.AddHours(8).AddSeconds(-5); // 限制 5 分鐘內不重複計數
