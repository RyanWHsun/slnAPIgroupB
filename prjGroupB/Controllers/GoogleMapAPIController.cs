    ...
    public class MapsController : ControllerBase {
        ...
        public async Task<IActionResult> Geocode([FromQuery] string address) {
            try {
                ...
                var googleApiUrl = $"https://maps.googleapis.com/maps/api/geocode/json?address={Uri.EscapeDataString(address)}&key={apiKey}";
                var response = await _httpClient.GetStringAsync(googleApiUrl);

                return Content(response, "application/json");
            }
            catch (HttpRequestException ex) {
                return StatusCode(500, new { error = "無法連線至 Google Maps API", details = ex.Message });
            }
            catch (Exception ex) {
        ...
        public async Task<IActionResult> GetMapData() {
            try {
                ...
                return StatusCode(500, new { error = "無法取得地圖資料", details = ex.Message });
            }
            catch (Exception ex) {
                return StatusCode(500, new { error = "內部錯誤", details = ex.Message });
            }
        ...
        public async Task<IActionResult> GetPlacePhotoAsync(string query) {
            try {
                ...
                // 1. 搜尋景點
                var searchUrl = $"{baseUrl}/textsearch/json?query={Uri.EscapeDataString(query)}&key={apiKey}";
        ...
        public async Task<IActionResult> GetLocations([FromBody] List<string> places) {
            try {
                ...
                foreach (var place in places) {
                    ...
                    if (!geocodeData.RootElement.TryGetProperty("results", out var results) || results.GetArrayLength() == 0) {
